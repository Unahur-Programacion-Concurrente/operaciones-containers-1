...
1. Revisar Prerrequisitos de Instalación   
   1. Requerimientos de SO  
      ...
      lsb\_release \-a  
   2. Desinstalar paquetes conflictivos (versiones antiguas)  
      for pkg in docker.io docker-doc docker-compose podman-docker containerd runc; do sudo apt-get remove $pkg; done  
        
2. Configurar repositorio apt de Docker  
   1. Instalar paquetes de requisitos previos para que apt pueda usar paquetes por HTTPS  
      sudo apt update  
      sudo apt install apt-transport-https ca-certificates curl software-properties-common  
   2. Instalar las claves de seguridad (GPG) oficiales de Docker  
      curl \-fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add \-  
   3. Agregar el repositorio de Docker en las fuentes (sources) de apt:  
